...
class ValueWrapperConfigurationTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $config = $this->create();
        $this->expectException(\Fgms\EmailInquiriesBundle\Configuration\Exception\NotLoadedException::class);
        $config->getKey();
    }
    ...
    public function testExecuteInvalidNoForm()
    {
        ...
        $this->expectException(\Fgms\EmailInquiriesBundle\Configuration\Exception\InvalidExecuteException::class);
        $config->execute(null);
    }
    ...
    {
        $config = $this->create((object)['id' => 5,'key' => 'foo']);
        $form = new \Fgms\EmailInquiriesBundle\Entity\Form();
